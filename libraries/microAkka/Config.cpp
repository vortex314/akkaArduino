...
void Config::clear() { _root = &_jsonBuffer.parseObject("{}");}
...
bool Config::hasKey(const char* key) {
	...
	if(ns.containsKey(key)) { return true; }
	return false;
}
...
void Config::load() {
	...
	if(!_root->success()) { _root = &_jsonBuffer.parseObject("{}"); }
}
...
void Config::save() {
	...
	if(err) { ERROR(" nvs_set_str(my_handle,  'config', buffer): %d ", err); }
